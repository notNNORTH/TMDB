...
public class BTree_Indexer<K, V> {
    ...
    private static class BTreeNode<K, V> {
        ...
        public BTreeNode(String fileName, long offset){
            ...
            // 首先读4字节判断节点的type
            int type = edu.whu.tmdb.level.Constant.BYTES_TO_INT(Constants.readBytesFromFile( offset, 4, fileName), 0, 4);
            offset += 4;
            ...
            if(type == 1){
                ...
                int entryCount = edu.whu.tmdb.level.Constant.BYTES_TO_INT(Constants.readBytesFromFile( offset, 4, fileName), 0, 4);
                offset += 4;
                // 每个Entry即每个k-v占用的字节数
                int singleEntryLength = edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH + Long.BYTES;
                // 总占用字节数
                ...
                for(int i=0; i<entryCount; i++){
                    byte[] b;
                    // K
                    b = new byte[edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH];
                    System.arraycopy(buffer, singleEntryLength * i, b, 0, edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH);
                    K k = (K) edu.whu.tmdb.level.Constant.BYTES_TO_KEY(b);
                    // V
                    b = new byte[Long.BYTES];
                    System.arraycopy(buffer, singleEntryLength * i + edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH, b, 0, Long.BYTES);
                    long v = edu.whu.tmdb.level.Constant.BYTES_TO_LONG(b);
                    // 添加entry
            ...
            // 非叶子节点
                ...
                // 读4字节即一个int，得到有多少个entry
                int entryCount = edu.whu.tmdb.level.Constant.BYTES_TO_INT(Constants.readBytesFromFile( entryStartOffset, 4, fileName), 0, 4);
                // 每个Entry即每个k-v占用的字节数
                int singleEntryLength = edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH + Long.BYTES;
                // Entry总占用字节数
                ...
                int childrenCount = edu.whu.tmdb.level.Constant.BYTES_TO_INT(Constants.readBytesFromFile( childrenStartOffset, 4, fileName), 0, 4);
                ...
                for(int i=0; i<childrenCount; i++){
                    ...
                    long address = edu.whu.tmdb.level.Constant.BYTES_TO_LONG(b);
                    // 递归
                ...
                for(int i=0; i<entryCount; i++){
                    ...
                    b = new byte[edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH];
                    System.arraycopy(buffer, singleEntryLength * i, b, 0, edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH);
                    String k = edu.whu.tmdb.level.Constant.BYTES_TO_KEY(b);
                    // V
                    b = new byte[Long.BYTES];
                    System.arraycopy(buffer, singleEntryLength * i + edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH, b, 0, Long.BYTES);
                    long v = edu.whu.tmdb.level.Constant.BYTES_TO_LONG(b);
        ...
        public long write(BufferedOutputStream outputStream, long offset) throws IOException {
            ...
            if(leaf){
                ...
                // 首部4字节记录type
                outputStream.write(edu.whu.tmdb.level.Constant.INT_TO_BYTES(1));
                // 再4字节记录k-v对的个数
                outputStream.write(edu.whu.tmdb.level.Constant.INT_TO_BYTES(entrys.size()));
                // 依次写入各Entry
                for(Entry entry : entrys){
                    // 写key
                    outputStream.write(edu.whu.tmdb.level.Constant.KEY_TO_BYTES(entry.key.toString()));
                    // 写value，其实是个记录offset的long
                    outputStream.write(edu.whu.tmdb.level.Constant.LONG_TO_BYTES((long)entry.value));
                }
                return totalLength;
            }
            ...
            // 非叶子节点
                ...
                // 首部4字节记录type
                outputStream.write(edu.whu.tmdb.level.Constant.INT_TO_BYTES(0));
                // 再4字节记录k-v对的个数
                outputStream.write(edu.whu.tmdb.level.Constant.INT_TO_BYTES(entrys.size()));
                // 依次写入各Entry
                for(Entry entry : entrys){
                ...
                // 再4字节记录children的个数
                outputStream.write(edu.whu.tmdb.level.Constant.INT_TO_BYTES(children.size()));
                // 依次写入各Children
                for(BTreeNode child : children){
                    // 每个child保存其offset
                    outputStream.write(edu.whu.tmdb.level.Constant.LONG_TO_BYTES((long)child.offset));
                }
                return totalLength;
            }
        }
    ...
    public long[] write(BufferedOutputStream outputStream, long offset){
        ...
        long totalSize;
        if(root.leaf){
            totalSize = Integer.BYTES * 2 + root.entrys.size() * (edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH + Long.BYTES);
        }
        else{
            totalSize = Integer.BYTES * 3 +
                    root.entrys.size() * (edu.whu.tmdb.level.Constant.MAX_KEY_LENGTH + Long.BYTES) +
